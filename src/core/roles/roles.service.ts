...
import { Role as Entity, RoleDocument as MongoDocument } from './schemas/role.schema';
...
export class RolesService implements IService {
  ...
  async list(pageOptionsDto: PageOptionsDto): TypeErrorOrPageDtoTuple<MongoDocument> {
    try {
      const [error, data] = await listMongoCollectionWithPagination(this.model, pageOptionsDto);
